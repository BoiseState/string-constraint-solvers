...
//            System.out.println(fileName);
    ...
    public static void runSolver3(DirectedGraph<PrintConstraint,
        ...
        LinkedList<PrintConstraint> toBeAdded =
                new LinkedList<PrintConstraint>();
        ArrayList<PrintConstraint> vertexSet =
        ...
        while (vertexSet.size() > 0) {
            ...
            if (toBeAdded.size() > 0) {
                ...
                if (ends.contains(first)) {
                    solve.addEnd(first.getSplitValue(),
                                 first.getActualVal(),
                                 first.getId(),
                                 first.getSourceMap());
                } else if (roots.contains(first)) {
                    solve.addRoot(first.getSplitValue(),
                                  first.getActualVal(),
                                  first.getId());
                    ...
                    solve.addOperation(first.getSplitValue(),
                                       first.getActualVal(),
                                       first.getId(),
                                       first.getSourceMap());
                }
