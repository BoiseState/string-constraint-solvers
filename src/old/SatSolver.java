...
//					    		  rt.exec("kill -9 "+pid);
		             ...
		             if (line !=null)
	 	  			...
	 	  			try{
		 	  			if(execResult!=null){
							double tempDouble=Double.parseDouble(execResult.split("Time\\(avg\\) =")[1].split("\\(s\\)")[0]);
							double reportedTime=tempDouble*Integer.parseInt(execResult.split("Runs: ")[1].split("\n")[0]);
	...
	protected String getBooleanResult(String fName, String baseGuess, String argGuess, HashMap<String, Integer> sourceMap){
		...
		if(fName.equals("contains")||fName.equals("contentEquals")||fName.equals("endsWith")||fName.equals("startsWith")||fName.equals("equals")||fName.equals("equalsIgnoreCase")||fName.equals("matches")){
			...
			if(sourceMap.size()==5){
				...
				boolean s1=Boolean.parseBoolean(actualVals.get(sourceMap.get("s1")));
