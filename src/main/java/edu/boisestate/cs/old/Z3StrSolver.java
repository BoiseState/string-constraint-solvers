	...
	public void addOperation(String string, String actualVal, int id,
				 ...
				 if(sourceMap.get("s1")==null){
					...
					if(sourceMap.size()>3){
						if(string.split("!!")[1].startsWith("[CI")){
							...
							int end=
									Integer.parseInt(actualVals.get(sourceMap.get("s3"))) - start;
							auto.add("(declare-variable in0_"+id+" String)\n");
							target="in0_"+id;
		...
		//TODO implement other copyValueOf
			...
			if(sourceMap.size()==2){
				...
				argTwo="i"+id;
				auto.add("(assert (= i"+id+" (- (Length s"+sourceMap.get("t")
						 +") "+(argOne)+")))\n");
			}
			else{
				argOne= Integer.parseInt(actualVals.get(sourceMap.get("s1")));
...
//	    }
	 ...
	 */
	private String getTargetString(int id, String solveString, String name) throws
																			Exception {
...
//			new Exception().printStackTrace();
	 ...
	 */
	protected ArrayList<String> changeCase(int id, HashMap<String, Integer> sourceMap, boolean toUpperCase, int source, String target, String caseName) throws
																																						Exception {
		char start, end;
		if(toUpperCase){
			start='a';
		...
		ArrayList<String>
				auto=new ArrayList<String>(((ArrayList<String>) store.get(source)));
		String targetVal;
		...
		for(int i=(int)start; i<=(int)end; i++){
			...
			for(int j=0; j<count; j++){
				...
				auto.add("(assert (= " + caseName + "_" + i + "_" + j + " (Replace " + caseName + "_" + i + "_" + (j-1) + " " + addExcapes(
						Character.toString(ch), auto, id) + " \"" + replacement + "\")))\n");
			}
			auto.add("(declare-variable "+caseName+"_"+(i+1)+"_"+(-1)+" String)\n");
			auto.add("(assert (= "+caseName+"_"+(i+1)+"_"+(-1)+" "+caseName+"_"+(i)+"_"+(count-1)+"))\n");
			auto.add("(assert (not (Contains " + caseName + "_" + (i+1) + "_" + (-1) + " " + addExcapes(
					Character.toString(ch), auto, id) + ")))\n");
		}
...
//		}
	...
	protected int testConstraint(int valId, int val2Id){
		ArrayList<String>
				base=new ArrayList<String>((ArrayList<String>)store.get(valId));
		if(val2Id>=0)
