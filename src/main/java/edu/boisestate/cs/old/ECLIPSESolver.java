...
public class ECLIPSESolver extends SatSolver{
	...
	protected ECLIPSESolver(boolean verbose, String properties, String tempFile) throws
...
//		System.err.println("Operation: "+string+" ID: "+id+" "+actualVal);
		...
		if((fName.equals("concatenate"))||fName.equals("concat")){
				...
				if(sourceMap.size()>3){
					if(string.split("!!")[1].startsWith("[CI")){
						...
						int argOne=
								Integer.parseInt(actualVals.get(sourceMap.get("s2"))) + 1;
						int argTwoInt=
		...
		//TODO implement other copyValueOf
			...
			numOperations--;
			 ...
			 String argTwo;
			int argOne=
					Integer.parseInt(actualVals.get(sourceMap.get("s1"))) + 1;
			if(sourceMap.size()==2){
...
//				target.add("(S"+id+"=[] ; str_substr(S"+sourceMap.get("t")+","+argOne+","+argTwo+", S"+id+"))");
		...
		//TODO implement other insert
			...
			if(string.split("!!")[1].equals("IC")){
				...
				int argOne=
						Integer.parseInt(actualVals.get(sourceMap.get("s3"))) + 1;
				int argTwoInt=
...
//			}
	 ...
	 */
	protected CompoundTerm rpc(int id, String term, boolean trackConstraintTime, boolean trackHotTime, boolean isTrueBranch) throws
																															 Exception {
