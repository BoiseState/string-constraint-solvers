...
public class ConcreteSolver
        extends ExtendedSolver<ConcreteValue>
    ...
    public void append(int id, int base, int arg) {
        ...
        if (baseCV.isFeasible() && argCV.isFeasible()) {
            ...
            if (baseCV.getValue() == null || argCV.getValue() == null) {
                //System.out.println("Map " + symbolicStringMap + " b " +
				// base + " a " + arg);
                //actual concrete value
                //System.out.println("acutal val for base " + Parser
				// .actualVals.containsKey(base) + " " + Parser.actualVals.get
				// (base));
                //System.out.println("acutal val for arg " + Parser
				// .actualVals.containsKey(arg) + " " + Parser.actualVals.get
				// (arg) +
                //		" result " + Parser.actualVals.containsKey(id) + " " +
				// Parser.actualVals.get(id));
                //We can create a new value based on the concrete one and
				// have arg
                //to have that concrete value
                String actualVal = Parser.actualVals.get(id);
                if (baseCV.getValue() != null && argCV.getValue() == null) {
                    String baseVal = baseCV.getValue();
                    //System.out.println("actualVal " + actualVal + " baseVal
					// " + baseVal);
                    //now get the proper offset
                    String retrivedVal = actualVal.substring(baseVal.length(),
                                                             actualVal.length
																	 ());
                    //System.out.println("retrived " + retrivedVal);
                    //fix arg
                    argCV.setValue(retrivedVal);
                } else if (baseCV.getValue() == null &&
                           argCV.getValue() != null) {
                    ...
                    System.err.println(
                            "Both base and arg are null - use concrete values" +
							" -- unhandeled");
                    System.exit(1);
                }
...
//			 if(id==106331){
//				 System.out.println(base + " " + baseStr + " " + arg + " " +
// argStr);
//			 }
    ...
    public void delete(int id, int base, int start, int end) {
        //		//first check for feasibility
        //				if(isFeasible(base)){
        //		StringBuilder baseStr = new StringBuilder(symbolicStringMap
		// .get(base));
    ...
    public void insert(int id, int base, int arg, int offset) {
        ...
        if (baseCV.isFeasible() && argCV.isFeasible()) {
            StringBuilder baseStr = new StringBuilder(baseCV.getValue());
            String argStr = argCV.getValue();
//					System.out.println("base " + Parser.actualVals.get(base) +
// " arg " + Parser.actualVals.get(arg) + " "
//							+ " offset " + offset + " idActual " + Parser
// .actualVals.get(id) + "\n basStr " + baseStr +
//							" argStr " + argStr);
...
//					System.exit(1);
    ...
    public void insert(int id,
		...
		// .get(base));
        //		String argStr = symbolicStringMap.get(arg);
        //		String resultStr = baseStr.insert(offset, argStr, start, end)
		// .toString();
        //		//put result into the map for that id
        ...
        if (baseCV.isFeasible() && argCV.isFeasible()) {
            ...
            String argStr = argCV.getValue();
            String resultStr =
                    baseStr.insert(offset, argStr, start, end).toString();
            //put result into the map for that id
            symbolicStringMap.put(id, new ConcreteValue(resultStr));
        } else {
            symbolicStringMap.put(id, new ConcreteValue());
...
//			System.exit(2);
    ...
    public void propagateSymbolicString(int id, int base) {
        ...
        //System.out.println("propagate " + base + " to " + id + " as " +
        // baseStr.getValue() + " to " +
    ...
    public void replaceStrings(int id, int base, int argOne, int argTwo) {
        //		if(isFeasible(base) && isFeasible(argOne) && isFeasible
		// (argTwo)){
        //		String baseStr = symbolicStringMap.get(base);
        //		String argOneStr = symbolicStringMap.get(argOne);
        //		String argTwoStr = symbolicStringMap.get(argTwo);
        //		symbolicStringMap.put(id, baseStr.replace(argOneStr,
		// argTwoStr));
        //
        //	} else {
        //		infeasible.add(id);
        ...
        if (baseCV.isFeasible() &&
            ...
            //put result into the map for that id
            symbolicStringMap.put(id,
                                  new ConcreteValue(baseStr.replace(argOneStr,
                                                                    argTwoStr)));
        } else {
            symbolicStringMap.put(id, new ConcreteValue());
        }
